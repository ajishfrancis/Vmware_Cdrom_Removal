...
# Ignore empty networks, this permits to keep networks when deploying a template/cloning a VM
        ...
        if nic_change_detected:
         ...
         #  self.change_detected = True
            ...
            msg="not changed"                
#self._edit(vim.vm.ConfigSpec(deviceChange=[nic]))
 #               return True
...
def main():
    ...
    argument_spec.update(
        ...
        nic_state=dict(type="str", default=True),
    )
        ...
        if module.params['state'] == 'present':
            if module.check_mode:
                result.update(
                    vm_name=vm.name,
                    changed=True,
                    desired_operation='reconfigure_vm1',
                )
                module.exit_json(**result)
            result = pyv.reconfigure_vm1()
